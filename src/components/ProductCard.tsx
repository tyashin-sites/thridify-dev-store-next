...
import { Card3DMedia } from '@/components/Card3DMedia';
  ...
  return (
    <a href={`/products/${p.slug}`} className="group card-hover block overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
      {/* Thridify listing-card marker — ONLY emitted on the listing/collection
          ...
          overlays a live (transparent) viewer on hover (desktop) and tears it
          down on leave. Card3DMedia (client) owns the per-card poster↔3D handoff
          — it hides this card's poster once the model paints so the poster does
          not bleed through behind the 3D, and restores it on hover-out. The
          overlay is pointer-events:none so this <a> always navigates to the PDP. */}
      {enable3d ? (
        <Card3DMedia p={p} />
      ) : (
